...
minimize Total_Cost: sum {s in SCENARIOS, p in PRODUCTS, m in MONTHS} (abs(cost[p,m]-costS[p,m,s])*(Make[p,m] + cond[p,m]*(Made[p,m]-maxno)*percent))*prob[s];

# (cost[p,m]*Make[p,m] + cond[p,m]*(Made[p,m]-maxno)*percent*cost[p,m]) + lmbd*(sum {s in SCENARIOS, p in PRODUCTS, m in MONTHS} (abs(cost[p,m]-costS[p,m,s])*(Make[p,m] + cond[p,m]*(Made[p,m]-maxno)*percent))*prob[s]);
