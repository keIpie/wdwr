...
minimize Total_Cost: sum {p in PRODUCTS, m in MONTHS} (cost[p,m]*Make[p,m] + cond[p,m]*(Make[p,m]-maxno)*percent*cost[p,m]);
...
subject to Storage {p in PRODUCTS, m in MONTHS}: Make[p,m] <= maxno + cond[p,m]*(promised[p]-maxno);
subject to Storage2 {p in PRODUCTS, m in MONTHS}: Make[p,m] >= maxno*cond[p,m];
